        ...
        {!loading && tab === 'bookings' && (
          <div style={s.list}>
            ...
            {bookings.map((b) => {
              ...
              return (
                <div key={b._id} className="booking-card" style={s.card}>
                  ...
                  {b.status === 'pending' && (
                    <div className="booking-actions" style={s.actionBtns}>
        ...
        {!loading && tab === 'tables' && (
          <div>
            <div style={s.addCard}>
              ...
              <div className="add-form" style={s.addForm}>
                <input
                  style={s.miniInput} placeholder="Table No." type="number"
                  value={newTable.tableNumber}
                  onChange={(e) => setNewTable({ ...newTable, tableNumber: e.target.value })} />
                <input
                  style={s.miniInput} placeholder="Capacity" type="number"
                  value={newTable.capacity}
                  onChange={(e) => setNewTable({ ...newTable, capacity: e.target.value })} />
                <select
                  style={s.miniInput} value={newTable.location}
            ...
            <div className="table-cards-grid" style={s.tableGrid}>
              {tables.map((t) => (
...
const s = {
  page: { background: '#0a0f1e', minHeight: '100vh', padding: 'clamp(80px, 10vw, 100px) clamp(1rem, 4vw, 2rem) 4rem', position: 'relative', overflow: 'hidden' },
  ...
  title: { fontFamily: "'Syne', sans-serif", fontSize: 'clamp(1.8rem, 4vw, 2.5rem)', fontWeight: '700', color: '#ffffff' },
  statsGrid: { display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem', marginBottom: '2rem' },
  statCard: { background: 'rgba(15,21,32,0.9)', border: '1px solid rgba(201,168,76,0.12)', borderRadius: '16px', padding: 'clamp(1rem, 3vw, 1.5rem)', position: 'relative', overflow: 'hidden' },
  statValue: { display: 'block', fontFamily: "'Syne', sans-serif", fontSize: 'clamp(1.6rem, 4vw, 2.2rem)', fontWeight: '700', color: '#c9a84c', lineHeight: 1, marginBottom: '0.4rem' },
  statLabel: { display: 'block', color: '#6b7a9e', fontSize: '0.78rem' },
  ...
  card: { background: 'rgba(15,21,32,0.9)', border: '1px solid rgba(201,168,76,0.08)', borderRadius: '14px', padding: '1.25rem 1.5rem', display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' },
  cardBody: { flex: 1, minWidth: '200px' },
  cardTop: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.4rem', gap: '0.5rem' },
  ...
  addCard: { background: 'rgba(15,21,32,0.9)', border: '1px solid rgba(201,168,76,0.15)', borderRadius: '16px', padding: '1.5rem', marginBottom: '2rem' },
  ...
  tableGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: '1rem' },
